...
function addSub(out: Float32Array, a: Float32Array, b: Float32Array, c: Float32Array){
	out[0] = a[0] + b[0] - c[0];
	out[1] = a[1] + b[1] - c[1];
}
...
export default class Narrowphase{
	 ...
	 */
	contactEquationPool: ContactEquationPool = new ContactEquationPool({ size: 32 });

	/**
	 * Keeps track of the allocated ContactEquations.
	 * @property {FrictionEquationPool} frictionEquationPool
	 */
	frictionEquationPool: FrictionEquationPool = new FrictionEquationPool({ size: 64 });
	 ...
	 */
	collidingBodiesLastStep: TupleDictionary = new TupleDictionary();
	...
	convexConvex(
		...
		let edgeA = findMaxSeparation(tempVec, polyA, positionA, angleA, polyB, positionB, angleB);
		let separationA = tempVec[0];
		if (separationA > totalRadius){
			return 0;
		}

		let edgeB = findMaxSeparation(tmpVec, polyB, positionB, angleB, polyA, positionA, angleA);
